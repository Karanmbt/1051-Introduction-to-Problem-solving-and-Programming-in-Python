...
def coin_flipping():
    ...
    for q in range(10):  #
        ...
        if result == "heads":  # if the result is "heads", increment the heads counter
            ...
            tails_total += 1  # if the result is "tails"
            # or anything else(which currently isn't possible as the only choices given to the random.choice are heads and tails
            # but if there were more then it would count as long as result didn't = heads),
            # increment the tails counter
